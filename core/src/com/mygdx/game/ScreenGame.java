...
public class ScreenGame implements Screen {
    ...
    public ScreenGame(GerusZArcade gerusZArcade) {
    ...
    private void spawnShots(){
        if(TimeUtils.millis() > timeLastShot+timeShotInterval) {
            ...
            if(gerusZArcade.isSoundOn) sndShot.play(0.05f);
        }
    }
    ...
    private void spawnFragments(SpaceObject object){
        if(gerusZArcade.isSoundOn) sndExplosion.play();
    ...
    private void gameOver(){
        isGameOver = true;
        players[players.length-1].name = gerusZArcade.playerName;
    ...
    void saveRecordsToDB(){
        ...
        // асинхронный запрос
        myApi.sendData(gerusZArcade.playerName, kills).enqueue(new Callback<List<RecordFromDB>>() {
            @Override
